...
var excalidraw_inline_default = 'function x(){let n=document.querySelector(".page[data-frame=\'excalidraw\']");if(n){b(n),L(n);return}let e=document.querySelectorAll(".excalidraw-page");for(let i of e)L(i)}function b(n){let e=n.querySelector(".excalidraw-sidebar-toggle");e&&(e.addEventListener("click",()=>{n.classList.toggle("excalidraw-sidebar-open")}),window.addCleanup(()=>{n.classList.remove("excalidraw-sidebar-open")}))}function L(n){let e=n.querySelector(".excalidraw-container");if(!e)return;let i=e.querySelector("svg");if(!i)return;e.style.backgroundColor="var(--excalidraw-bg, var(--light))";let a=1,s=0,c=0,l=!1,d=0,u=0;function o(){i.style.transform=`translate(${s}px, ${c}px) scale(${a})`}function m(t){let r=t;for(;r&&r!==e;){if(r.classList&&(r.classList.contains("excalidraw-embed-note")||r.classList.contains("excalidraw-embed-url")))return!0;r=r.parentElement}return!1}function f(t){if(m(t.target))return;t.preventDefault();let r=t.deltaY>0?-.15:.15;a=Math.max(.1,Math.min(5,a+r)),o()}function v(t){t.button===0&&(m(t.target)||(l=!0,d=t.clientX-s,u=t.clientY-c,e.style.cursor="grabbing"))}function M(t){l&&(s=t.clientX-d,c=t.clientY-u,o())}function w(){l=!1,e.style.cursor="grab"}let O=n.querySelector(".excalidraw-zoom-in"),E=n.querySelector(".excalidraw-zoom-out"),g=n.querySelector(".excalidraw-reset");O&&O.addEventListener("click",()=>{a=Math.min(5,a+.15),o()}),E&&E.addEventListener("click",()=>{a=Math.max(.1,a-.15),o()}),g&&g.addEventListener("click",()=>{a=1,s=0,c=0,o()}),e.addEventListener("wheel",f,{passive:!1}),e.addEventListener("mousedown",v),document.addEventListener("mousemove",M),document.addEventListener("mouseup",w),window.addCleanup(()=>{e.removeEventListener("wheel",f),e.removeEventListener("mousedown",v),document.removeEventListener("mousemove",M),document.removeEventListener("mouseup",w)})}document.addEventListener("nav",x);\n';
var l3;
