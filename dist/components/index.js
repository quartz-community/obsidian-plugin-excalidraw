...
function renderEmbeddable(el, resolvedEmbeds) {
  const link = el.link ?? "";
  const isWikilink = link.startsWith("[[");
  const label = link.replace(/^\[\[/, "").replace(/\]\]$/, "").replace(/^https?:\/\//, "");
  const truncatedLabel = label.length > 50 ? label.slice(0, 47) + "..." : label;
  const resolved = resolvedEmbeds?.[el.id];
  if (isWikilink) {
    const noteContent = resolved ? `<a href="${escapeAttr(resolved.href)}" class="excalidraw-embed-open-link">Open note \u2192</a><div class="excalidraw-embed-body">${resolved.html}</div>` : `<span class="excalidraw-embed-missing">Note not found</span>`;
    return [
      ...
      `<div class="excalidraw-embed-header">\u{1F4C4} ${escapeXml(truncatedLabel)}</div>`,
  ...
  return [
    ...
    `<a href="${escapeAttr(link)}" target="_blank" rel="noopener noreferrer">\u{1F517} ${escapeXml(truncatedLabel)}</a>`,
    `</div>`,
...
var excalidraw_inline_default = 'function b(){let n=document.querySelector(".page[data-frame=\'excalidraw\']");if(n){h(n),x(n);return}let t=document.querySelectorAll(".excalidraw-page");for(let i of t)x(i)}function h(n){let t=n.querySelector(".excalidraw-sidebar-toggle");t&&(t.addEventListener("click",()=>{n.classList.toggle("excalidraw-sidebar-open")}),window.addCleanup(()=>{n.classList.remove("excalidraw-sidebar-open")}))}function x(n){let t=n.querySelector(".excalidraw-container");if(!t)return;let i=t.querySelector("svg");if(!i)return;t.style.backgroundColor="var(--excalidraw-bg, var(--light))";let a=1,s=0,l=0,d=!1,m=0,f=0;function o(){i.style.transform=`translate(${s}px, ${l}px) scale(${a})`}function v(e){if(!e)return!1;let r=e;for(;r&&r!==t;){if(r.tagName==="foreignObject"||r.tagName==="FOREIGNOBJECT"||r.classList&&(r.classList.contains("excalidraw-embed-note")||r.classList.contains("excalidraw-embed-url")))return!0;r=r.parentElement||r.parentNode}return!1}for(var u=t.querySelectorAll(".excalidraw-embed-note, .excalidraw-embed-url"),c=0;c<u.length;c++)u[c].addEventListener("wheel",function(e){e.stopPropagation()},{passive:!0}),u[c].addEventListener("mousedown",function(e){e.stopPropagation()});function w(e){if(v(e.target))return;e.preventDefault();let r=e.deltaY>0?-.15:.15;a=Math.max(.1,Math.min(5,a+r)),o()}function O(e){e.button===0&&(v(e.target)||(d=!0,m=e.clientX-s,f=e.clientY-l,t.style.cursor="grabbing"))}function E(e){d&&(s=e.clientX-m,l=e.clientY-f,o())}function M(){d=!1,t.style.cursor="grab"}let g=n.querySelector(".excalidraw-zoom-in"),p=n.querySelector(".excalidraw-zoom-out"),L=n.querySelector(".excalidraw-reset");g&&g.addEventListener("click",()=>{a=Math.min(5,a+.15),o()}),p&&p.addEventListener("click",()=>{a=Math.max(.1,a-.15),o()}),L&&L.addEventListener("click",()=>{a=1,s=0,l=0,o()}),t.addEventListener("wheel",w,{passive:!1}),t.addEventListener("mousedown",O),document.addEventListener("mousemove",E),document.addEventListener("mouseup",M),window.addCleanup(()=>{t.removeEventListener("wheel",w),t.removeEventListener("mousedown",O),document.removeEventListener("mousemove",E),document.removeEventListener("mouseup",M)})}document.addEventListener("nav",b);\n';
var l3;
    ...
    return /* @__PURE__ */ u4(
      "article",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ u4("div", { class: "excalidraw-container", dangerouslySetInnerHTML: { __html: svgContent } }),
