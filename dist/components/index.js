...
var excalidraw_inline_default = `function D(){let t=document.querySelector(".page[data-frame='excalidraw']");if(t){N(t),Z(t);return}let e=document.querySelectorAll(".excalidraw-page");for(let d of e)Z(d)}function N(t){let e=t.querySelector(".excalidraw-sidebar-toggle");e&&(e.addEventListener("click",()=>{t.classList.toggle("excalidraw-sidebar-open")}),window.addCleanup(()=>{t.classList.remove("excalidraw-sidebar-open")}))}function Z(t){let e=t.querySelector(".excalidraw-container");if(!e)return;let d=e.querySelector("svg");if(!d)return;e.style.backgroundColor="var(--excalidraw-bg, var(--light))";var o=t.querySelector(".excalidraw-overlays");let r=1,l=0,s=0,v=!1,m=0,g=0;function w(){if(o){var a=o.querySelectorAll(".excalidraw-overlay");if(a.length!==0)for(var u=parseFloat(o.getAttribute("data-viewbox-w"))||1,E=parseFloat(o.getAttribute("data-viewbox-h"))||1,_=parseFloat(o.getAttribute("data-offset-x"))||0,q=parseFloat(o.getAttribute("data-offset-y"))||0,S=e.getBoundingClientRect(),L=S.width,A=S.height,i=Math.min(L/u,A/E),X=(L-u*i)/2,F=(A-E*i)/2,f=0;f<a.length;f++){var n=a[f],P=parseFloat(n.getAttribute("data-x"))||0,C=parseFloat(n.getAttribute("data-y"))||0,Y=parseFloat(n.getAttribute("data-w"))||0,T=parseFloat(n.getAttribute("data-h"))||0,k=X+(P+_)*i,z=F+(C+q)*i,B=Y*i,I=T*i;n.style.left=k*r+l+"px",n.style.top=z*r+s+"px",n.style.width=B*r+"px",n.style.height=I*r+"px",n.style.display="flex"}}}w();function c(){d.style.transform="translate("+l+"px, "+s+"px) scale("+r+")",w()}function y(a){a.preventDefault();var u=a.deltaY>0?-.15:.15;r=Math.max(.1,Math.min(5,r+u)),c()}function p(a){a.button===0&&(v=!0,m=a.clientX-l,g=a.clientY-s,e.style.cursor="grabbing")}function b(a){v&&(l=a.clientX-m,s=a.clientY-g,c())}function x(){v=!1,e.style.cursor="grab"}var M=t.querySelector(".excalidraw-zoom-in"),O=t.querySelector(".excalidraw-zoom-out"),h=t.querySelector(".excalidraw-reset");M&&M.addEventListener("click",function(){r=Math.min(5,r+.15),c()}),O&&O.addEventListener("click",function(){r=Math.max(.1,r-.15),c()}),h&&h.addEventListener("click",function(){r=1,l=0,s=0,c()}),e.addEventListener("wheel",y,{passive:!1}),e.addEventListener("mousedown",p),document.addEventListener("mousemove",b),document.addEventListener("mouseup",x),window.addCleanup(function(){e.removeEventListener("wheel",y),e.removeEventListener("mousedown",p),document.removeEventListener("mousemove",b),document.removeEventListener("mouseup",x)})}document.addEventListener("nav",D);
`;
var l3;
